...
; For size reasons, I opted to use registers optimized
; for assembly instructions for function calls, rather
; than the C/C++ function calling convention.
;
; rsi		- stores the pointer of the string we're working with
; rdi		- the destination string, if manipulating data
; rax		- used for return values, and the first number parameter.
; r8-r12	- used for other parameters
;
; all other modified registers are saved, except rdi, rax, r11-r12
...
;
;DCE note: These are tightly coupled and small enough that
;optimizing them for DCE actually could make larger code with
...
;	rax, length of data to write
;	r8, the file handle 
; using rax as the file handle would be more efficient...
; except our strlen functions return the length in rax.
; Returns RAX:
...
write:
	push rdx	;
	mov rdx,rax
	mov rdi,r8	; set the file handle
...
;	rax, length of data to read
;	r8, file handle
; Returns:
;	rax positive number of bytes read (or 0 for none)
; 	rax negative means there was an error.
global read
read:
	push rdx	; 1 byte
	mov rdx,rax	; 3 bytes
	mov rdi,r8	; 3 bytes, set the file handle
	xor eax,eax 	; 2 bytes, zero rax
	;mov al,0	; 0 is the opt code for write rax is already zero.

	push rcx	; 1 bytes syscall always destroys rcx
	syscall		; 2 bytes
	pop rcx		; 1 byte
...
; and smaller strlen function above.
...
cstrlen_fast
	...
	repne scasb	; 2 bytes, keep looping until it finds the byte in al.
			; this uses microcode that takes 15-20 ticks to 
